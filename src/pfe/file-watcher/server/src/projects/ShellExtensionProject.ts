...
export class ShellExtensionProject implements IExtensionProject {
    ...
    private setLanguage = async (projectInfo: ProjectInfo): Promise<void> => {
        ...
        try {
            const result = await processManager.spawnDetachedAsync(
                projectInfo.projectID,
                path.join(this.path, "entrypoint.sh"),
                args,
                {});
    ...
    init = async (projectInfo: ProjectInfo): Promise<void> => {
        this.path = projectInfo.extensionID;
        this.config = await fs.readJson(path.join(this.path, ".sh-extension"));
    ...
    create = (operation: Operation): void => {
        projectUtil.containerCreate(operation, path.join(this.path, "entrypoint.sh"), "create");
    }
    ...
    update = (operation: Operation, changedFiles?: projectEventsController.IFileChangeEvent[]): void => {
        projectUtil.containerUpdate(operation, path.join(this.path, "entrypoint.sh"), "update");
    }
    ...
    start = async (projectInfo: ProjectInfo): Promise<void> => {
        await projectUtil.runScript(projectInfo, path.join(this.path, "entrypoint.sh"), "start");
    }
    ...
    stop = async (projectInfo: ProjectInfo): Promise<void> => {
        await projectUtil.runScript(projectInfo, path.join(this.path, "entrypoint.sh"), "stop");
    }
    ...
    rebuild = async (projectInfo: ProjectInfo): Promise<void> => {
        await projectUtil.runScript(projectInfo, path.join(this.path, "entrypoint.sh"), "rebuild");
    }
    ...
    deleteContainer = async (projectInfo: ProjectInfo): Promise<void> => {
        await projectUtil.containerDelete(projectInfo, path.join(this.path, "entrypoint.sh"));
    }
