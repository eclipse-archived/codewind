...
export function projectSpecificationTest(socket: SocketIO, projData: ProjectCreation, projectLang: string): void {
    describe("projectSpecification function", () => {
        ...
        _.forEach(combinations, (combo) => {
            ...
            it(`config project specification settings ${setting}`, async () => {
                if (setting === "internalPort") {
                    ...
                    if (process.env.IN_K8) {
                        ...
                        const podPorts = internalPorts.podPorts;
                        value = podPorts.length > 0 ? podPorts[Math.floor(Math.random() * podPorts.length)] : project_configs.defaultInternalPorts[projectLang];
                    } else {
                        const containerName = await projectUtil.getContainerName(projectInfo);
                        const internalPorts = await getApplicationContainerInfo(projectInfo, containerName);
                        const containerPorts = internalPorts.containerPorts;
                        value = containerPorts.length > 0 ? containerPorts[Math.floor(Math.random() * containerPorts.length)] : project_configs.defaultInternalPorts[projectLang];
                    }
                }
