...
function killServerProcesses() {
    pid="$(pgrep microclimatetestswift)"
    if [ "$pid" != "" ]
    then
        echo "killing microclimatetestswift"
        kill $pid
    fi
