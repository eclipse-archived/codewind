...
function killServerProcesses() {
    pid="$(pgrep swiftfolderimporttest)"
    if [ "$pid" != "" ]
    then
        echo "killing swiftfolderimporttest"
        kill $pid
    fi
