...
public class GoProjectCache extends AbstractMicroclimateTest {
	
	private static String exposedPort;
	private static String projectName = "gocache";
	...
	public void TestAcreate(){
		...
		try {
			...
			Logger.println(GoProjectCache.class, "TestAcreate()", "HttpResult is: " + HttpResult);
			assertTrue(HttpResult == HttpURLConnection.HTTP_ACCEPTED);
		}catch(Exception e) {
			Logger.println(GoProjectCache.class, "TestAcreate()", "Exception occurred during project creation: " + e.getMessage(),e);
	...
	public void TestBcheckForContainer() {
		try {
			...
			Logger.println(GoProjectCache.class, "TestCcheckForContainer()", "Exposed Port is " + exposedPort);
			assertNotNull("exposedPort for project " + projectName +" is null", exposedPort);
		}catch(Exception e) {
			Logger.println(GoProjectCache.class, "TestCcheckForContainer()", "Exception occurred when looking for exposedport: " + e.getMessage(),e);
	...
	public void TestCcheckEndpoint() {
		...
		try {
			...
			Logger.println(GoProjectCache.class, "TestEcheckEndpoint()", "Exception occurred when checking for endpoint",e);
